...
class TrackViewer:
    def __init__(self, window):
        ...
        self.artists_filter_combobox = ttk.Combobox(controls, width = 22, textvariable = self.artists_filter_input, values = ["All artists", *lib.list_artists()], state = "readonly").grid(row = 1, column = 5, pady = 4)
    ...
    def list_tracks(self):
        set_text(self.list_text, lib.list_all())
    ...
    def view_tracks(self):
        ...
        if not name:
            set_text(self.detail_text, f"Track {track_number} not found")
        ...
        set_text(self.detail_text, track_details)
    ...
    def search_tracks(self):
        ...
        result = lib.search_tracks(query)
        ...
        if not result:
            set_text(self.list_text, "No tracks matched")
            self.status_text.set("0 tracks matched with request")
            return
        
        set_text(self.list_text, result)
